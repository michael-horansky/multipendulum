...
\begin{document}
  ...
  $$\delta \vec{s}^* (\vec{Q}, c) = \mqty( c^{-1}\left(\left(R^c(\vec{w}_n)\right)^{-1} \vec{Q}\right)_\omega \\[5pt] \left(\left(R^c(\vec{w}_n)\right)^{-1} \vec{Q}\right)_{\vec{u}^*})$$
	...
	\subsection{TODO}
	\subsubsection{Theoretical thingies}
	The displaced mode should have a frequency matching the frequency of the driving force - we need to find an intersection in the pertrurbed modes and the allowed frequencies (which is a horizontal slice through frequency-mode space)
	
	Then as the force omega increases we need to maximize on this intersection? Lagrange multiplier? Or do we have to use system-inherent scale after all?? HELP
	
	1. for a given force Q, omegq: find displaced mode that mathces omega (this determines c)
	2. find energy of mode
	3. maximise energy w.r.t. omega
	4. this will be resonant omega??
	
	remember: the displaced mode is an ALLOWED MODE WITH THOSE CONSTRAINT FORCES. this should allow for BOTH MODE VECTOR AND FREQUENCY
	
	\subsubsection{Experimental thingies}
	You have the actual lagrangian $L$ and the approximated quadratic Lagrangian $L_2$, both with their respective equations of motion.
	
	TODO: Draw the response curve of both and compare the peaks. The $L_2$ curve SHOULD diverge at natural frequencies; the $L$ curve should exhibit resonance perturbations (as it does). Quantify shift with scale? Quantify the effective Hamiltonian?
